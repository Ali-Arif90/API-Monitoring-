...
// Dashboard-compatible self-service routes for authenticated client users/admins
router.get("/client/api-keys", (req, res, next) => {
    req.params.clientId = req.user?.clientId;
    return clientController.getClientApiKeys(req, res, next);
});

router.post("/client/api-keys", (req, res, next) => {
    req.params.clientId = req.user?.clientId;
    return clientController.createApiKey(req, res, next);
});
